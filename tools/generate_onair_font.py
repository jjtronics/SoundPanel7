...
FONT_PATH = "DIN-Condensed-Bold"
OUTPUT_PATH = ROOT / "src" / "ui" / "fonts" / "sp7_font_onair_220.c"
POINT_SIZE = 304
FONT_NAME = "sp7_font_onair_220"
SYMBOLS = ["A", "F", "I", "N", "O", "R"]
...
def generate() -> str:
    ...
    bitmap_index = 0
    for char, width, height, pixels in rendered:
        ...
        glyph_metrics.append((box_h, ofs_y))
    ...
    ascent = max(box_h + max(ofs_y, 0) for box_h, ofs_y in glyph_metrics)
    descent = max(max(-ofs_y, 0) for box_h, ofs_y in glyph_metrics)
    line_height = ascent + descent
    base_line = descent

    codepoints = [ord(ch) for ch in SYMBOLS]
    range_start = min(codepoints)
    range_length = (max(codepoints) - range_start) + 1
    unicode_offsets = [cp - range_start for cp in codepoints]

    return f"""/*******************************************************************************
 * Size: {POINT_SIZE} px
 * Bpp: 4
 * Opts: generated locally from {FONT_PATH} for symbols {' '.join(SYMBOLS)}
******************************************************************************/
...
static const uint16_t unicode_list_0[] = {{
    {", ".join(f"0x{offset:x}" for offset in unicode_offsets)}
}};
