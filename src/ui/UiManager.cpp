...
void UiManager::buildDashboard() {
  ...
  lv_obj_t* gear = lv_btn_create(top);
  ...
  // ===== Zone haute =====
  lv_obj_t* upper = lv_obj_create(_scrDash);
  lv_obj_set_size(upper, lv_pct(100), 242);
  lv_obj_align(upper, LV_ALIGN_TOP_MID, 0, 68);
  ...
  // ----- Carte heure gauche (style broadcast)
  lv_obj_t* clockCard = makeCard(upper, 170, 220);
  ...
  lv_obj_set_size(rightCol, 170, 220);
  ...
  lv_obj_t* c1 = makeCard(rightCol, 170, 103);
  lv_obj_t* c2 = makeCard(rightCol, 170, 103);
  ...
  // ===== Historique =====
  _histWrap = lv_obj_create(_scrDash);
  lv_obj_set_size(_histWrap, lv_pct(92), 118);
  lv_obj_align(_histWrap, LV_ALIGN_BOTTOM_MID, 0, -54);
  lv_obj_set_style_bg_color(_histWrap, lv_color_hex(0x111824), 0);
  lv_obj_set_style_border_width(_histWrap, 0, 0);
  lv_obj_set_style_radius(_histWrap, 18, 0);
  lv_obj_set_style_pad_all(_histWrap, 10, 0);
  lv_obj_clear_flag(_histWrap, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_set_scrollbar_mode(_histWrap, LV_SCROLLBAR_MODE_OFF);

  _lblHist = lv_label_create(_histWrap);
  lv_label_set_text(_lblHist, "Historique 5 min");
  lv_obj_set_style_text_font(_lblHist, &lv_font_montserrat_16, 0);
  lv_obj_set_style_text_color(_lblHist, lv_color_hex(0x8EA1B3), 0);
  lv_obj_align(_lblHist, LV_ALIGN_TOP_LEFT, 0, 0);

  lv_obj_t* histArea = lv_obj_create(_histWrap);
  lv_obj_set_size(histArea, lv_pct(100), 74);
  ...
  lv_obj_set_style_radius(histArea, 12, 0);
  lv_obj_set_style_pad_left(histArea, 6, 0);
  lv_obj_set_style_pad_right(histArea, 6, 0);
  ...
  // Repères temps (sous le graphe, mais toujours DANS la carte)
  _lblHistTLeft = lv_label_create(_histWrap);
  lv_label_set_text(_lblHistTLeft, "-5m");
  lv_obj_set_style_text_font(_lblHistTLeft, &lv_font_montserrat_16, 0);
  lv_obj_set_style_text_color(_lblHistTLeft, lv_color_hex(0xFFFFFF), 0);
  lv_obj_align(_lblHistTLeft, LV_ALIGN_BOTTOM_LEFT, 10, -4);

  _lblHistTMid = lv_label_create(_histWrap);
  lv_label_set_text(_lblHistTMid, "-2m");
  lv_obj_set_style_text_font(_lblHistTMid, &lv_font_montserrat_16, 0);
  lv_obj_set_style_text_color(_lblHistTMid, lv_color_hex(0xFFFFFF), 0);
  lv_obj_align(_lblHistTMid, LV_ALIGN_BOTTOM_MID, 0, -4);

  _lblHistTRight = lv_label_create(_histWrap);
  lv_label_set_text(_lblHistTRight, "0");
  lv_obj_set_style_text_font(_lblHistTRight, &lv_font_montserrat_16, 0);
  lv_obj_set_style_text_color(_lblHistTRight, lv_color_hex(0xFFFFFF), 0);
  lv_obj_align(_lblHistTRight, LV_ALIGN_BOTTOM_RIGHT, -10, -4);

  for (uint16_t i = 0; i < HISTORY_BAR_COUNT; i++) {
    _histBars[i] = lv_obj_create(histArea);
    lv_obj_set_width(_histBars[i], 5);
    lv_obj_set_height(_histBars[i], 4);
    lv_obj_set_style_radius(_histBars[i], 2, 0);
    lv_obj_set_style_bg_color(_histBars[i], lv_color_hex(0x23C552), 0);
    lv_obj_set_style_border_width(_histBars[i], 0, 0);
    lv_obj_clear_flag(_histBars[i], LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(_histBars[i], LV_SCROLLBAR_MODE_OFF);
  }

  // ===== Footer =====
  lv_obj_t* bottom = lv_obj_create(_scrDash);
  lv_obj_set_size(bottom, lv_pct(100), 44);
...
void UiManager::setDb(float dbInstant, float leq, float peak) {
  ...
  lv_label_set_text(_lblDb, buf);

  snprintf(buf, sizeof(buf), "%.1f", leq);
  lv_label_set_text(_lblLeq, buf);

  snprintf(buf, sizeof(buf), "%.1f", peak);
  lv_label_set_text(_lblPeak, buf);
  ...
  lv_obj_set_style_bg_color(_dot, c, 0);
...
void UiManager::tick() {
  ...
  if (_lblClockDate && _lblClockMain && _lblClockSec) {
    if (hasTime) {
      ...
      lv_label_set_text(_lblClockDate, dateBuf);
      lv_label_set_text(_lblClockMain, mainBuf);
      lv_label_set_text(_lblClockSec, secBuf);
    } else {
      lv_label_set_text(_lblClockDate, "--/--/----");
      lv_label_set_text(_lblClockMain, "--:--");
      lv_label_set_text(_lblClockSec, ":--");
